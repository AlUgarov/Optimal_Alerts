...
To estimate this correlation, we estimate a Tobit model of WTP on signal characteristics, i.e., false positive and false negative rate. We convert these rates into false positive (negative) costs by multiplying the false positive (negative) rate with the expected cost from an incorrect action from purchasing (not purchasing) protection.  That is, the false positive cost = $FP = P_{10}.[(1 -\pi).c]$, while the false negative cost = $FN = P_{01}.(\pi.L)$. Note that these costs already account for expected frequency of receiving different incorrect signals as consistent with their base rate. 

\begin{itemize}
	\item \agt{ADD TOBIT SPECIFICATION EQUATION}
	\item \agt{We can discuss this, but not sure what we need to include. Also, I know we discussed this, but I forgot what Belief change or Certainty is here. What's the story that needs telling?}
\end{itemize}

Table~\ref{tab:tobitMain} presents the results. Discuss the results (DO WE NEED THIS?).
