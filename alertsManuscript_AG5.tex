...
\adjustbox{max width = \textwidth}{
	...
	\begin{tabular}{l*{6}{c}}
		\hline\hline
		&\multicolumn{4}{c}{\multirow{2}{*}{All}} & \multicolumn{2}{c}{[\dots] Median Accuracy}\\ \cmidrule(lr){6-7}
		&&&&& Above & Below \\ 
...
We use data from the BE task to categorize subjects' belief accuracy. We construct two measures of belief accuracy, i.e., accuracy by subject-treatment (where a treatment is a combination of prior, false positive and false negative rate); and accuracy by subject. We define a belief error as the absolute value of the difference between the subject's belief and the true posterior probability. We categorize a subject as accurate for a treatment if their belief error is less than 0.005 standard deviation (sd) of belief error distribution for that treatment. Meanwhile, for the second measure, we categorize a subject to be an accurate (i.e., above median) Bayesian updater if the number of time they are categorized as inaccurate by treatment is less than the median across all subjects.

We find that discriminating by subjects' ability to perform Bayesian updating (hereafter, belief accuracy) increases the precision of the estimates. Column 3 presents the heterogeneity by subject-treatment belief accuracy. We find that on average, when subjects accurately update their belief for an honest treatment, their WTP aligns with the signal value, while inaccurate subjects tend to underpay for an honest signal. We also find that incorporating belief accuracy improved the statistical precision of the estimates for all coefficients. When we examine the heterogeneity by subject belief accuracy (column 4), we find a similar improvement in statistical precision. Above-median accuracy subjects slightly underpaid for honest signals, but this may be due to their updating mistakes for particular treatments.

Finally, we separately study the role of risk preference for more v. less accurate subjects. Accurate, risk-neutral subjects' WTP align well with the theoretical signal value, and they do not systematically overpay for signals with false positive and/or false negative rate. Consistent with Proposition~\ref{thm:riskAverse} \agt{XXX ALEX, is this true? I am not entirely sure what ``more sensitive'' mean here in terms of how it would affect the sign on the coefficient XXX}, accurate risk-averse subjects are willing to pay a premium for false negative signals;  however, they are unwilling to pay a premium for false positive signals. Risk-loving accurate subjects are willing to pay an even higher premium for false negative signals. In contrast, inaccurate subjects underpay for honest signals. The behavior of the risk-averse inaccurate subjects is similar to their accurate counterparts. On the other hand, risk-loving inaccurate subjects are willing to pay premiums for signals with higher false negative and false positive rates. 
