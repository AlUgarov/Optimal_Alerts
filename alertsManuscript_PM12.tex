...
\adjustbox{max width = \textwidth}{
	...
	\end{tabular}
	\begin{tablenotes}[flushleft]
		\item\leavevmode\kern-\scriptspace\kern-\labelsep \small\textit{Notes:} Standard errors in parentheses.  \textit{green}{The reported p-values are for tests of the hypothesis that...} \sym{*} \(p<0.10\), \sym{**} \(p<0.05\), \sym{***} \(p<0.01\).
	\end{tablenotes}
	\end{threeparttable}
}
\end{table}		
...
TO further explore this hypothesis, in Table~\ref{tab:protectReg} \textcolor{green}{We need to make sure wre using the same number of significant digits through out the table.  They currently go from 2 to 5 significant digits.}, we regress IP decisions on FP and FN rates and flexible controls of both posteriors and reported beliefs:\footnote{Given that the true functional form is unknown, we use a linear probability model to get unbiased coefficient estimates.}
