...
\paragraph{Probability Weighting} 
...
\paragraph{Bias.} Given that both Informed Protection and WTP task responses exhibit certan biases, it begs a question on whether the WTP bias can be explained by the IP bias. In other words, do subjects adjust their bids to account for projected distortions in signal's use? We find that their Informed Protection biases do affect their bid choices, but their explanatory power falls short of explaining the main patterns of sensitivities tp FP and FN costs. 

In order to study the bias, we create two variables measuring respectively how the signal availability changes subjects' expected protection costs and expected losses using their eliicited BP ($a_i$) and IP responses ($a_{iw}$, $a_{ib}$ for respectively white and black signals)\footnote{Given that there is no elicited blind protection choice for prior of $\pi=0.5$, we assume full protection. For context, \% subjects protect when facing prior 0.3}: 

\[
\Delta^{\text{prot}}_{i}\equiv c\Big[a_i-\big(\pi(P_{11}a_{iw}+P_{01}a_{ib})+(1-\pi)(P_{10}a_{iw}+P_{00}a_{ib})\big)\Big]
\]

\[
\Delta^{\text{loss}}_{i}\equiv L\pi\Big[(1-a_i)-\big(P_{11}(1-a_{iw})+P_{01}(1-a_{ib})\big)\Big]
\]

The first component in square brackets represents blind protection, while the second component represents informed protection. The sum of these variables equals to the total expected benefit of having a signal given the strategy, but splitting the benefit in two parts allows to account asymmetry in responding to large losses versus relatively small protection costs.

In Table X in Appendix we report estimation results of WTP difference with added controls of elicited expected protection and losses associated with actual subject's choices. The coefficients on new expected costs variables are strongly statistically significant but reatively small in magnitude. The magnitude of coefficients on FP and FN costs reduces by roughly one third in estimations including all priors, but both coefficients on FP costs for low priors and FN costs for high priors remain highly statistically significant. This indicates that subject's biases do not explain the observed pattern of sensitivities to FP and FN rates.
...
\adjustbox{max width = \textwidth}{
	\begin{threeparttable}
	\caption{WTP and Signal Characteristics, first rounds}
	\label{tab:wtp_ols}
	\begin{tabular}{l*{5}{c}}
		\hline\hline
		&\multicolumn{3}{c}{\multirow{2}{*}{All}} & \multicolumn{2}{c}{Prior}\\ \cmidrule(lr){5-6}
	...
	\hline
	\\ [-0.5em]
		\exInput{Tables/wtpdiff_ols_firstrounds.tex}
				\hline
		Subject FE & Yes & Yes & Yes & Yes & Yes \\
		Inaccurate Belief Interactions & No & No & Yes & Yes & Yes \\
		Prior Probability FE & No & No & No & Yes & Yes \\
