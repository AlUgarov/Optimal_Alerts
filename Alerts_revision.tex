...
\paragraph{Bias.} Given that both Informed Protection and WTP task responses exhibit certan biased, it raises a question on whether the WTP bias can be explained by the IP bias. This assumes that subjects are aware of their biases which can happen in two different scenarios: subjects are aware both of the presence and direction of the bias, or subjects are only aware of the bias magnitude but not direction. For example, subjects might not know whether dusty outside air increases or decreases smoke alarm FP and FN rates and hence have lower willingness-to-pay for smoke alarms close to construction zones. While the first type of bias awareness is possible, it often requires the presence of significant 'irrational' factors affecting decisions (such as utility from holding certain beliefs), and testing it seems impossible without accepting a specific theory of the bias. Hence, we concentrate on the first type of bias, as it much easier to test and doesn't require a 

We observe that subjects overprotect in response to white signals in informed protection, and underprotect in response to black signals. This pattern would reduce signal's value, and particularly for information structure with high priors and significant positive values. This can provide at least partial explanation for lower senstivity of WTP to FP and FN costs as compared to theoretical predictions.

We find that the bias in IP provides little explanatory power for observed WTP sensitivity patterns. We calculate the measure of subjects' gain from having signal access accounting for their actual blind and informed protection decisions\footnote{The expected gain is calculated as the differnce between projected BP and IP protectio losses in the flollowing way: $\hat V_i=[ac+(1-a)pL]-[\pi(P_{11}(a_b c+(1-a_b) L)+P_{01}(a_w c+(1-a_w)L)+(1-\pi)(P_{00} a_w c+P_{10} a_b c)]$. Here $a$ is their reported blind protection action for a given prior $\pi$ and $a_w, a_b$ are the reported IP actions in response to white and black signals.} When we control for the subject's projected signal's gain in WTP regression, we find that the while the expected gain is singificant predictor, the magnitude of other coefficient estimates is practically unaffected.
