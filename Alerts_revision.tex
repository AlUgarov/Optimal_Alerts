...
\aut{Without restriction of generality, we assume information structure in which a positive signal $s=1$ is more likely in the adverse state of the world $P_{11}>P_{10}$.}

\vspace{10pt}
\paragraph{Strategy.} \aut{Without a signal, the agent can choose only between always protecting and never protecting resulting in the following expected utility:
\[
...
\[U(\sigma=(0,0),b)=\pi u(Y-L-b)+(1-\pi) u(Y-b), U(\sigma=(1,1),b)=u(Y-c-b) \]

If a subject follows the signal her expected utility is:
\[
U(\sigma=(0,1),b)=\pi P_{11}u(Y-c-b)+\pi P_{01}u(Y-L-b)+(1-\pi)P_{10}u(Y-c-b)+(1-\pi)P_{00}u(Y-b)
\]

Given our assumptions, the expected utility of following the signal is higher than the expected utility of following the "do the opposite" strategy:
 \[
U[\sigma=(1,0),b]=\pi P_{01}u(Y-c-b)+\pi P_{11}u(Y-L-b)+(1-\pi)P_{00}u(Y-c-b)+(1-\pi)P_{10}u(Y-b)
\]

Hence, value of the signal or the maximum willingness to pay for it is determined by comparing the expected utility of following the signal with the expected utility of not using a signal:
\begin{equation}
\max_\sigma U(\sigma,b)=U_0=\max[u(Y-c),\pi u(Y-L)+(1-\pi) u(Y)]
\end{equation}

The signal's value cannot be positive if it is not followed (because for $b>0$ $\max[U(\sigma=(0,0),b), U(\sigma=(1,1),b))<U_0$), hence it should satisfy:
...
\paragraph{Risk-neutral agent.} If the agent is risk-neutral, the expression (X) above collapses to:
...
We can express the WTP for the signal, $b$, as a function of priors, false-positive (FP), and false-negative rates (FN) denoted correspondingly as $P_{10}$ and $P_{01}$. The following equation serves as our benchmark in the empirical work:
\begin{equation}
b=\max[0,\min[c,\pi L]-\pi (1-P_{01})c-(1-\pi)P_{10}c-\pi P_{01}L]
\end{equation}\label{eq:rnWTP}
The signal's value is decreasing in both FP and FN rates. The effect is proportional to the non-adverse (adverse) state probability for the false-positive (false-negative) rate. When WTP is positive ($b>0$),  its derivatives with respect to FP ($P_{10}$) and FN ($P_{01}$) rates are given by:
...
Note also that willingness-to-pay is always equally affected by \textbf{FP and FN costs} which are defined as $\pi (1-P_{01})c$ and $\pi P_{01}L$.
...
\begin{theorem}
 If an adverse state probability is high, (i.e., $\pi>L/c$), then a strictly risk-averse decision-maker pays less than a risk-neutral one.
\end{theorem} 
\begin{proof}
See the Appendix.
\end{proof}
...
\bigskip
\noindent\textbf{Informed Protection (IP)}.\ \ \ Similar to the BP task, subjects must make a protection decision given the prior probability of drawing a black ball. Subjects learn a prior and signal's \agst{accuracy}\agt{information structure}. Following \citet{coutts_good_2019}, \agt{we represent this information structure using a group of hinting gremlins, where the hint is provided by a randomly selected gremlin (mapping to a signal realization in the model)}. The gremlin is one of three types: (i) honest;\footnote{\agt{XXX If we want, we can discuss in this footnote that the term "honest" likely describes how people perceive the signal in real life, assuming we can find good anecdotes or studies XXX}} (ii) ''black-swamp'' who always says that the ball is black; and (iii) ''white-swamp'' who always says that the ball is white. Figure~\ref{fig:Gremlins} illustrates how the different gremlin types were presented to the subjects. The composition of the group of gremlins determines \agst{signal's accuracy}\agt{the information structure}: a higher share of black(white)-swamp gremlins produces a signal with higher FP (FN) rate. Subjects know the group composition, but do not know which gremlin provides a hint in any particular round. 
\agst{The prior probability of drawing a black ball and the composition of gremlins vary across the rounds.}  
...
As in the IP task, subjects know the prior probability of drawing a black ball and the \agt{information structure that is represented by the composition of the group of gremlins}. Instead of making a protection decision, however, subjects are asked to estimate the probability that: (i) the ball is black when the gremlin says that it is white; (ii) the ball is black when the gremlin says that it is black. 
...
\paragraph{Willingness to Pay Elicitation (WTPE).} The WTPE task measures a subject's willingness to pay (WTP) for a signal. As before, subjects know the prior probability of drawing a black ball and the \agst{composition of the group of gremlins giving hints}\agt{information structure}.  Unlike the IP task, subjects do not automatically receive a hint, instead they provide their WTP for a hint by choosing a value $\in (\$0,\$5)$ in \$0.50 increments. The elicitation is incentive compatible: if a WTPE round is selected as the payment round, a random price of a hint will be drawn. If that price exceeds the subject's WTP, they will play a BP round, otherwise the subject pays their WTP and plays an IP round.  

\paragraph{\agt{Post-Experiment Questionnaire and Payment.}}  After the WTPE task, subjects answered a few demographic questions: gender, age, and the number of statistics courses they have taken.\footnote{\agt{XXX DO WE need to mention the additional demographic questions on the second wave? XXX}} The payment task and the payment round were then randomly chosen to calculate the subject's payoff. 

\paragraph{\agt{Treaments and Treatment Assignment.}}\agt{Table~\ref{tab:treatments} summarizes our treatments for the tasks other than BP. The treatments combine four priors with 12 different information structures for a total of 48 treatments. Figure XX shows that with these treatments, we can recover a distribution of posteriors (for black and white signals) that spans a wide range of interior values and allows for a wide range of theoretical WTP values.}

\agt{Subjects were assigned to treatments in the following manner. Each subject was randomly assigned six treatments that combined two priors with three information structures. The treatments were organized by the priors and the order of the priors --- i.e., whether the first was larger than the second --- was randomly assigned. For each prior, the first treatment was always the one with a fully truthful information structure. The order of priors and signals stays constant for each subject across tasks, but can vary between subjects. }
...
\input{Tables/table_treatments_new.tex}
\end{table}
...
\agt{Given the complexities of some of these tasks, we use this section to show the extent to which subjects understand each task using simple descriptive analyses. We find that subjects understand these tasks reasonably well. This section also provides a qualitative overview of how subjects tend to deviate from the theoretical predictions for each task. We defer the more formal regression analysis of how signal characteristics affect subjects' WTP to  Section~\ref{sec:results}.} 
...
\adjustbox{max width=\textwidth}{
	...
	\hline \hline
		...
		\hline	
\input{Tables/bigpicture_bel_AU_cl.tex}
\\ 	[-1em]
\hline\hline
\end{tabular} 
	\begin{tablenotes}[flushleft]
...
Table~\ref{tab:nonparError} summarizes how updating errors vary with the \agst{signal's characteristics}\agt{information structure}. We find that subjects overestimate the probability of a black ball when receiving a white \agst{hint}\agt{signal}, which is consistent with overprotection noted for the IP task. This upward bias for a white \agst{hint}\agt{signal} increases in both the FP and FN rates of the \agst{signal}\agt{information structure}. To illustrate, consider the change between rows 1 and 3, where introducing an FP rate would not change the posterior because the signal \agst{realization} is white. Yet, subjects update their posterior upward, magnifying their updating error; we find a similar effect for the introduction of the FN rate (row 1 vs. 2).

The updating bias for black signals\agst{ realizations (hints)}, however, varies by the information structure. Subjects slightly underestimate the probability with a perfectly accurate signal, but introducing FN rates exacerbates subjects' underestimation. Rows 5 and 6 suggest that the introduction of an FN rate without changing the posterior further reduces subjects' beliefs. With non-zero FP rate, subjects again overestimate the probability of a black ball. The difference in the updating errors for black hints coming from FP-only (row 7) v. FP-FN signals (row 8) is negligible. 
...
\begin{center} \agt{* * *} \end{center}
\noindent\agt{IS IT WORTH SUMMARIZING HOW "SENSIBLE" OUR SUBJECTS FOR EACH TASK HERE?}
...
\adjustbox{max width=\textwidth}{
	...
	\end{tabular}
	\begin{tablenotes}[flushleft]
			\item\leavevmode\kern-\scriptspace\kern-\labelsep \footnotesize \textit{Notes:} Standard errors in parentheses (clustered at the subject level). 
	\end{tablenotes}								
	\end{threeparttable}
	}
\end{table}
...
\adjustbox{max width=\textwidth}{
	...
	\end{tabular}
	\begin{tablenotes}[flushleft]
			\item\leavevmode\kern-\scriptspace\kern-\labelsep \footnotesize \textit{Notes:} Coefficients are average marginal effects. Standard errors in parentheses (clustered at the subject level). 
	\end{tablenotes}								
	\end{threeparttable}
	}
\end{table}
...
Overall, we observe a striking uniformity in sensitivity of WTP to both false-positive and false-negative rates that cannot be explained by risk preferences or anchoring. This pattern is, however, consistent with subjects neglecting the difference between false-positive and false-negative signals, a behavior that is supported by subjects' explanations of their decision making and the odd sensitivities to false-positive and false-negative rates in other treatments in which they do not affect posterior probabilities.\footnote{This pattern is consistent with greater bias in belief elicitation when subjects have to engage in contingent reasoning versus smaller belief biases when eliciting responses after presenting a signal results, as \citet{aina_contingent_2023} found. This result implies that decisions to acquire information, such as decisions made in our experiment where subjects determining their WTP have to reason through contingencies, might suffer from persistent inherent biases. Indeed, we find that subjects commit reasoning errors which reduces the correlation between their WTP for a signal and its usefulness for decreasing expected potential costs.} 



\paragraph{Bias.} Given that both Informed Protection and WTP task responses exhibit certan biases, it begs a question on whether the WTP bias can be explained by the IP bias. In other words, do subjects adjust their bids accounting to projected distortions in signal's use. This assumes that subjects are aware of their biases which can happen in two different scenarios: subjects are aware both of the presence and direction of the bias, or subjects are only aware of the bias magnitude but not direction. An example of the first bias would be evacuating when hearing a smoke alarm while knowing it is triggered by your own smoking. As an example of the second type of bias, subjects might not know whether dusty outside air increases or decreases smoke alarm FP and FN rates and hence have lower willingness-to-pay for smoke alarms close to construction zones. While the first type of bias awareness is possible, it often requires the presence of significant 'irrational' factors affecting decisions (such as utility from holding certain beliefs), and testing it seems impossible without assuming a specific theory of the bias.

Note that any bias in informed protection can only We observe that subjects overprotect in response to white signals in informed protection, and underprotect in response to black signals. This pattern would reduce signal's value, and particularly for information structures with high priors and significant positive values. This can provide at least partial explanation for lower senstivity of WTP to FP and FN costs as compared to theoretical predictions.


We find that the bias in IP provides little explanatory power for observed WTP sensitivity patterns. We calculate how much subjects gain from having signal access accounting for their actual blind and informed protection decisions\footnote{The expected gain is calculated as the differnce between projected BP and IP protection losses in the flollowing way: $\hat V_i=[ac+(1-a)pL]-[\pi(P_{11}(a_b c+(1-a_b) L)+P_{01}(a_w c+(1-a_w)L)+(1-\pi)(P_{00} a_w c+P_{10} a_b c)]$. Here $a$ is their reported blind protection action for a given prior $\pi$ and $a_w, a_b$ are the reported IP actions in response to white and black signals.} Note that this value does not account for risk aversion. 

We also find that subjects pay significantly less for signals associated with high errors conditional on other signal's characteristics. Increasing error by one standard deviation corresponds to a decrease in WTP by roughly sixteen cents (pretty small though given that average WTP is about 2.5 USD). 

When we control for the subject's projected signal's gain in WTP regression, we find that the while the expected gain is significant predictor of willingness-to-pay, the magnitude of other coefficient estimates is practically unaffected. This value

However incresing the value of information 
...
\adjustbox{max width=\textwidth}{
	\begin{threeparttable}	
	\input{Tables/table_be3_AU.tex}
	\begin{tablenotes}[flushleft]
			\item\leavevmode\kern-\scriptspace\kern-\labelsep \footnotesize \textit{Notes:} Decomposition works only for imperfect signals, hence the table excludes the responses to certain signals. Standard errors in parentheses (clustered at the subject level). 
	\end{tablenotes}								
	\end{threeparttable}
	}
\end{table}
...
\adjustbox{max width=\textwidth}{
	\begin{threeparttable}	
	\input{Tables/table_ip5_AU.tex}
	\begin{tablenotes}[flushleft]
			\item\leavevmode\kern-\scriptspace\kern-\labelsep \footnotesize \textit{Notes:} Specifications include flexible controls of posterior probability. The table reports average marginal effects, includes subject FE, errors are clustered by subject. Standard errors in parentheses.
	\end{tablenotes}								
	\end{threeparttable}
	}
\end{table}
...
\adjustbox{max width=\textwidth}{
	\begin{threeparttable}	
	\input{Tables/table_wtpdiff_02_AU.tex}
	\begin{tablenotes}[flushleft]
			\item\leavevmode\kern-\scriptspace\kern-\labelsep \footnotesize \textit{Notes:} Standard errors in parentheses (clustered by subject).
	\end{tablenotes}								
	\end{threeparttable}
	}
\end{table}
...
\adjustbox{max width = \textwidth}{
	\begin{threeparttable}
	\caption{WTP and Signal Characteristics (Tobit)}
	\label{tab:wtp_ols}
	\begin{tabular}{l*{5}{c}}
		\hline\hline
		&\multicolumn{3}{c}{\multirow{2}{*}{All}} & \multicolumn{2}{c}{Prior}\\ \cmidrule(lr){5-6}
...
\adjustbox{max width = \textwidth}{
	\begin{threeparttable}
	\caption{WTP and Signal Characteristics (Tobit), IP bias}
	\label{tab:wtp_ols}
	\begin{tabular}{l*{5}{c}}
		\hline\hline
		&\multicolumn{3}{c}{\multirow{2}{*}{All}} & \multicolumn{2}{c}{Prior}\\ \cmidrule(lr){5-6}
	...
	\hline
	\\ [-0.5em]
		\exInput{Tables/wtp_tobit_valdiff.tex}
				\hline
		Subject FE & Yes & Yes & Yes & Yes & Yes \\
		Inaccurate Belief Interactions & No & No & Yes & Yes & Yes \\
		Prior Probability FE & No & No & No & Yes & Yes \\
...
Because \aut{$u(\dot)$ is an increasing function we obtain $b<b_0$}. \end{proof} 
...
\aut{Sensitivities to FP and FN rates are given as:
