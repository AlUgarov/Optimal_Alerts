...
\title{\Large Preference for Quality-Adjusted Warning Signals: \\ Experimental Evidence\\}
...
We use a laboratory experiment to examine preferences over these trade-offs and study the demand for information of a known quality for a protection decision.
...
Subjects receive information about the prior probability of an adverse event and decide whether to take a protective action after receiving a signal with known false-positive and negative rates. We then elicit their willingness to pay (WTP) for each signal. 

Compared to a risk-neutral decision maker, subjects prefer incurring higher false-positive costs from a warning system. At the same time, their WTP is weakly correlated with the value of information: their WTP fails to fully respond to changes in signal quality, resulting in the overpayment of low-quality signals and underpayment of high-quality signals. Notably, we find asymmetric (under-)responsiveness by prior: 
...
We find that this pattern is most consistent with a failure to estimate the effect of the frequencies of false-positive and false-negative outcomes on the potential costs of using the signal. 
...
\bibliographystyle{aea} % Style BST file
