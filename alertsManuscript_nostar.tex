...
\title{\Large Preferences for Warning Signal Quality: Experimental Evidence\thanks{The
experiment in this paper was approved by the Institutional Review Board (IRB) of the
University of Arkansas (\#2105335334).}}
...
\onehalfspacing
\begin{abstract}{
\noindent We use a laboratory experiment to study preferences over false-positive and false-negative rates of warning signals for an adverse event with a known prior. We find that demand does not adequately respond to changes in signal quality. Subjects disproportionately reduce their demand for signals with higher false-negative rates for rare events, while the opposite holds true for frequent events. We show that neither risk preference nor Bayesian updating skills can fully explain our results. Our results are most consistent with a decision-making heuristic in which subjects do not distinguish between false-positive and false-negative errors. 
...
\adjustbox{max width=\textwidth}{
	...
	\input{Tables/table_be_err_AU_nostar.tex}
	\\ [-1em]
	\hline
...
\adjustbox{max width=\textwidth}{
	...
	\hline
		...
		\input{Tables/table_ip_flex_AU_nostar.tex}
...
\adjustbox{max width=\textwidth}{
	\begin{threeparttable}	
	\input{Tables/wtp_het_risk_AU.tex}
	\begin{tablenotes}[flushleft]
			\item\leavevmode\kern-\scriptspace\kern-\labelsep \footnotesize \textit{Notes:} Standard errors in parentheses (clustered by subject).
	\end{tablenotes}								
	\end{threeparttable}
