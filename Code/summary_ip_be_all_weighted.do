...
replace bp = bp_2 if abs(p-0.10)<0.01   // 0.10 -> bp_2
replace bp = bp_4 if abs(p-0.20)<0.01   // 0.20 -> bp_4
replace bp = bp_6 if abs(p-0.30)<0.01    // 0.30 -> bp_6
replace bp = 1  if abs(p-0.50)<0.01 // p=0.5 -> always protect

cap drop bp_val
gen bp_val=-(p*(1-bp)*loss+bp*protectioncost)
*Extra protection costs of BP:
gen ip_prot = protectioncost*(bp-p*(phintWB*ip_w+phintBB*ip_b)-(1-p)*(phintWW*ip_w+phintBW*ip_b))
*Extra losses of BP:
gen ip_loss = loss*p*((1-bp)-(phintWB*(1-ip_w)+phintBB*(1-ip_b)))
...
sum ip_prot
sum ip_loss
reg ip_val bp_val ip_prot ip_loss
gen ip_val_norm=max(0,ip_val)
