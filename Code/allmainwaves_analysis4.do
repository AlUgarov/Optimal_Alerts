...
eststo clear
eststo: reg ip_ i.strategy_short##c.phintWB i.strategy_short##c.phintBW if plevel<300, vce(cluster subject_id)
eststo: reg ip_ i.strategy_short##c.phintWB i.strategy_short##c.phintBW if hint=="w"&plevel<300, vce(cluster subject_id)
eststo: reg ip_ i.strategy_short##c.phintWB i.strategy_short##c.phintBW if hint=="b"&plevel<300, vce(cluster subject_id)
...
reg ip_ i.strategy_short##c.phintBW if hint=="w"&phintWB==0, vce(cluster subject_id)
...
gen sort_violation=0
replace sort_violation=1 if ((meanip<meanip[_n-1])&(subject_id==subject_id[_n-1])&hint=="w"&hint[_n-1]=="w")
replace sort_violation=1 if ((meanip>meanip[_n-1])&(subject_id==subject_id[_n-1])&hint=="b"&hint[_n-1]=="b")
replace sort_violation=1 if hint=="w"&false_prob==0&meanip>0
replace sort_violation=1 if hint=="b"&false_prob==0&meanip<1
bys subject_id: egen honest_following=max(sort_violation)
replace honest_following=1-honest_following
tab honest_following

collapse (mean) honest_following, by(subject_id)
save "./Temp/honest_following.dta", replace
use "./Temp/strat_analysis.dta", replace
merge m:1 subject_id using "./Temp/honest_following.dta"
drop _merge

*Study the association between self-reported and actual strategies:
tab strategy_ip hint_follower, row nofreq chi2
tab strategy_ip bayesian, row nofreq chi2
tab strategy_ip overprotect, row nofreq chi2
tab bayesian honest_following, row nofreq chi2
tab honest_following overprotect, row nofreq chi2


*Findings: self-reported strategies are strongly associated with actual strategies but the link is not obvious
*Groups 5 and 6 follow hints the most (group 5 includes self-repored hint followers so it's not surprising)
*Group 1 is not the most consistent with Bayesian decision-making. Groups 4 and 5 are the most consistent, followed by the group 1, though the differences are not drastic.
*Subjects from groups 4 and 5 also more likely to protect in response to a white signal when there are no white-eyed gremlin (so the posterior prob is 0)

*Who are not Bayesian?

*Not protecting when black but protecting (sometimes) when white:
gen black_noprotect0=(ip_==0)&(hint=="b")&avprot_w>0
bys subject_id: egen black_noprotect=max(black_noprotect0)
tab black_noprotect bayesian

tab black_noprotect bel_follower
tab black_noprotect bayesian if overprotect==0
...
eststo clear
eststo: probit ip_ post_prob0* bespline* phintBW phintWB if plevel<300, vce(cluster subject_id)
eststo m1: margins, dydx(phintBW phintWB) post
eststo: probit ip_ post_prob0* bespline* phintBW phintWB i.subject_id if plevel<300, vce(cluster subject_id)
eststo m2: margins, dydx(phintBW phintWB) post
eststo: probit ip_ post_prob0* bespline* highprob phintBW phintWB high_phintBW high_phintWB i.subject_id if plevel<300, vce(cluster subject_id)
eststo m3: margins, dydx(highprob phintBW phintWB high_phintBW high_phintWB) post
eststo: probit ip_ post_prob0* bespline* blackhint phintBW phintWB black_phintBW black_phintWB i.subject_id if plevel<300, vce(cluster subject_id)
eststo m4: margins, dydx(blackhint phintBW phintWB black_phintBW black_phintWB) post
eststo: probit ip_ post_prob0* bespline* phintBW phintWB if blackhint==0&plevel<300, vce(cluster subject_id)
eststo m5: margins, dydx(phintBW phintWB) post
eststo: probit ip_ post_prob0* bespline* phintBW phintWB if blackhint==1&plevel<300, vce(cluster subject_id)
eststo m6: margins, dydx(phintBW phintWB) post
