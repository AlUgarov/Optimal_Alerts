...
merge m:1 subject_id round using "./Temp/bel_accuracy_all.dta" //average belief accuracy by subject
drop _merge
...
***Indicator of unused signals:
gen unused=(ip_w==ip_b)&!missing(ip_w)&!missing(ip_b)
gen positive_wtp=wtp>0
label def unused_lab 0 "Protection affected by signal" 1 "Signal has no effect"
label value unused unused_lab

label def positive_wtp_lab 0 "WTP$=$0" 1 "WTP$>$0"
...
table unused positive_wtp, c(freq mean wtp) name(tbl1, replace)
...
sum wtp if unused==0 & positive_wtp==0, detail
matrix `T'[1,1] = r(N)
matrix `T'[1,2] = r(mean)


sum wtp if unused==0 & positive_wtp==1, detail
matrix `T'[1,3] = r(N)
matrix `T'[1,4] = r(mean)


sum wtp if unused==1 & positive_wtp==0, detail
matrix `T'[2,1] = r(N)
matrix `T'[2,2] = r(mean)


sum wtp if unused==1 & positive_wtp==1, detail
matrix `T'[2,3] = r(N)
matrix `T'[2,4] = r(mean)

*------------------------------------------------------------*
* Move matrix to dataset
*------------------------------------------------------------*
clear
svmat double `T', names(T)

gen rowlab = cond(_n==1, ///
    "Protection affected by signal", ///
    "Signal has no effect")
order rowlab

*------------------------------------------------------------*
* Export LaTeX table (same layout as original)
*------------------------------------------------------------*
#delimit ;
listtex rowlab T1 T2 T3 T4 using "./Tables/protection_to_wtp.tex", ///
    replace rstyle(tabular) ///
    head("\begin{tabular}{lcccc}
\\toprule
& \\multicolumn{2}{c}{WTP \$=\$ 0} & \\multicolumn{2}{c}{WTP \$>\$ 0} \\\\
...
hist wtp_diff, xtitle("USD") fraction color(navy)
...
hist wtp_diff if fp_env==0&fn_env==0, xtitle("USD") fraction color(navy)
graph export "./Graphs/hist_WTP_discr1h.png", width(1200) height(800) replace

hist wtp_diff if fp_env==1&fn_env==0, xtitle("USD") fraction color(navy)
graph export "./Graphs/hist_WTP_discr1fp.png", width(1200) height(800) replace

hist wtp_diff if fp_env==0&fn_env==1, xtitle("USD") fraction color(navy)
graph export "./Graphs/hist_WTP_discr1fn.png", width(1200) height(800) replace

hist wtp_diff if fp_env==1&fn_env==1, xtitle("USD") fraction color(navy)
...
* Global binning so all four charts are comparable
quietly sum wtp_diff, meanonly
scalar NBINS = 20
scalar WTP_MIN = r(min)
scalar WTP_MAX = r(max)

gen bin = floor((wtp_diff-WTP_MIN)/(WTP_MAX-WTP_MIN)*NBINS)
replace bin = NBINS - 1 if bin == NBINS

scalar BINWIDTH = (WTP_MAX - WTP_MIN)/NBINS


gen x_mid = WTP_MIN + (WTP_MAX - WTP_MIN)*(bin+0.5)/NBINS

preserve
    collapse (sum) w_treat, by(bin x_mid)

    egen totw = total(w_treat)
    gen frac = w_treat/totw

    twoway bar frac x_mid, ///
	    barwidth(`=BINWIDTH') ///
        xtitle("USD") ///
        ytitle("Fraction") ///
        color(navy)

    graph export "./Graphs/hist_WTP_discr1_w.png", ///
...
preserve
    ...
    graph export "./Graphs/hist_WTP_discr1h_w.png", ///
        width(1200) height(800) replace
restore
...
preserve
    ...
    graph export "./Graphs/hist_WTP_discr1fp_w.png", ///
        width(1200) height(800) replace
restore
...
preserve
    ...
    graph export "./Graphs/hist_WTP_discr1fn_w.png", ///
        width(1200) height(800) replace
restore
...
preserve
    ...
    graph export "./Graphs/hist_WTP_discr1fpfn_w.png", ///
        width(1200) height(800) replace
restore
...
local binwidth = 0.05  // adjust depending on your data range

twoway ///
(histogram value if wave==1, width(`binwidth') fcolor(navy) lcolor(navy)) ///
(histogram value if wave==2, width(`binwidth') fcolor("220 50 80") lcolor("220 50 80")), ///
legend(order(1 "Wave 1" 2 "Wave 2") position(1) ring(0) ///
       region(lcolor(none) lwidth(0) fcolor(none))) ///
title("Distribution of signal's value for a risk-neutral subject") ///
xtitle("USD") ///
ytitle(Fraction) ///
...
twoway ///
(histogram value if (fp_env==1 & fn_env==1), width(`binwidth') fcolor("220 50 80") lcolor("220 50 80")) ///  // Both errors
(histogram value if ((fp_env==1 & fn_env==0) | (fp_env==0 & fn_env==1)), width(`binwidth') fcolor(navy) lcolor(navy)), ///  // Single error
legend(order(1 "Both errors (FP & FN)" 2 "Single error (FP or FN)") position(1) ring(0) ///
       region(lcolor(none) lwidth(0) fcolor(none))) ///
title("Distribution of signal's value by error type") ///
xtitle("USD") ///
ytitle(Fraction) ///
note("Value = expected change in costs from BP to IP")

graph export "./Graphs/hist_value_by_error.png", width(1200) height(800) replace
...
eststo clear
eststo: ivreg2 wtp_diff i.highprob##c.false_neg i.highprob##c.false_pos, cluster(subject_id treatn)
...
eststo clear
eststo: ivreg2 wtp_diff consistent_wtp i.highprob##c.false_neg i.highprob##c.false_pos, cluster(subject_id treatn)
eststo: ivreg2 wtp_diff consistent_wtp i.highprob##c.false_neg consistent_wtp#i.highprob#c.false_neg i.highprob##c.false_pos c.consistent_wtp#i.highprob#c.false_pos, cluster(subject_id treatn)
eststo: ivreg2 wtp_diff consistent_wtp c.consistent_wtp#i.highprob##c.false_neg c.consistent_wtp##i.highprob##c.false_pos, cluster(subject_id treatn)
eststo: reghdfe wtp_diff consistent_wtp i.highprob##c.false_neg consistent_wtp#i.highprob#c.false_neg i.highprob##c.false_pos c.consistent_wtp#i.highprob#c.false_pos, abs(subject_id) vce(cluster subject_id treatn)
eststo: reghdfe wtp_diff consistent_wtp c.consistent_wtp#i.highprob##c.false_neg c.consistent_wtp#i.highprob##c.false_pos, abs(subject_id) vce(cluster subject_id treatn)
esttab using "./Tables/wtp_consistent.tex", b(%9.3g) se(%9.1f) ar2(%9.2f) label drop(_cons) title(WTP minus Value of Information, risk aversion and sensitivity to FP and FN costs) mtitles("" "" "" "FE" "FE") star("*" 0.10 "**" 0.05 "***" 0.01) nobaselevels compress nogaps replace
esttab using "./Tables/wtp_consistent_pres.tex", b(%9.3g) ar2(%9.2f) not label drop(_cons) indicate("Prior-FP interactions=*.highprob#c.false_pos" "Prior-FN interactions=*.highprob#c.false_neg") mtitles("" "" "" "FE" "FE") star("*" 0.10 "**" 0.05 "***" 0.01) nobaselevels compress nogaps replace
...
eststo clear
eststo: ivreg2 wtp i.plevel fp_cost fn_cost false_pos2 false_neg2, cluster(subject_id treatn)
estadd scalar adj_r= e(r2_a)
test false_pos2 false_neg2
local p=r(p)
estadd scalar p = `p'
eststo: ivreg2 wtp i.plevel fp_cost fn_cost i.plevel#c.fp_cost i.plevel#c.fn_cost, cluster(subject_id treatn)
estadd scalar adj_r= e(r2_a)
...
local p=r(p)
estadd scalar p = `p'
eststo: reghdfe wtp i.plevel fp_cost fn_cost false_pos2 false_neg2, absorb(subject_id) vce(cluster subject_id treatn)
estadd scalar adj_r= e(r2_a)
test false_pos2 false_neg2
local p=r(p)
estadd scalar p = `p'
eststo: reghdfe wtp i.plevel fp_cost fn_cost i.plevel#c.fp_cost i.plevel#c.fn_cost, absorb(subject_id) vce(cluster subject_id treatn)
estadd scalar adj_r= e(r2_a)
test 200.plevel#c.fp_cost 200.plevel#c.fn_cost 300.plevel#c.fp_cost 300.plevel#c.fn_cost 500.plevel#c.fp_cost 500.plevel#c.fn_cost
local p=r(p)
estadd scalar p = `p'
...
eststo clear
eststo: ivreg2 wtp i.plevel phintWB phintBW, cluster(subject_id treatn)
test phintWB=phintBW
estadd scalar adj_r= e(r2_a)
estadd scalar p = r(p)
eststo: ivreg2 wtp i.plevel i.plevel#c.phintWB i.plevel#c.phintBW, cluster(subject_id treatn)
...
estadd scalar adj_r= e(r2_a)
estadd scalar p = r(p)
eststo: reghdfe wtp i.plevel phintWB phintBW,  absorb(subject_id) vce(cluster subject_id treatn)
test phintWB=phintBW
estadd scalar adj_r= e(r2_a)
estadd scalar p = r(p)
eststo: reghdfe wtp i.plevel i.plevel#c.phintWB i.plevel#c.phintBW,  absorb(subject_id) vce(cluster subject_id treatn)
test  (100.plevel#c.phintWB=100.plevel#c.phintBW) (200.plevel#c.phintWB=200.plevel#c.phintBW)  (300.plevel#c.phintWB=300.plevel#c.phintBW)   (500.plevel#c.phintWB=500.plevel#c.phintBW)
estadd scalar adj_r= e(r2_a)
estadd scalar p = r(p)
...
eststo: reghdfe wtp_diff false_pos false_neg if round>3, abs(subject_id) vce(cluster subject_id treatn)
    estadd scalar F_value = e(F)
	test false_pos = false_neg
	estadd scalar test_eq=r(p)
	testparm *
...
eststo: reghdfe wtp_diff i.risk_pref##c.false_pos i.risk_pref##c.false_neg if round<4, abs(subject_id) vce(cluster subject_id treatn)
    estadd scalar F_value = e(F)
	testparm *
    estadd scalar p_value=r(p)
	test false_pos = false_neg
...
eststo: reghdfe wtp_diff i.risk_pref##i.inac_bel2##c.false_pos i.risk_pref##i.inac_bel2##c.false_neg if round<4, abs(subject_id) vce(cluster subject_id treatn)
    estadd scalar F_value = e(F)
		testparm *
    estadd scalar p_value=r(p)
	test false_pos = false_neg
...
eststo: reghdfe wtp_diff i.risk_pref##i.inac_bel2##c.false_pos i.risk_pref##i.inac_bel2##c.false_neg if !phigh & round<4, abs(subject_id plevel) vce(cluster subject_id treatn)
    estadd scalar F_value = e(F)
		testparm *
    estadd scalar p_value=r(p)
		test false_pos = false_neg
...
#delimit ;
	esttab * using "./Tables/wtpdiff_ols_firstrounds.tex", replace label 
...
eststo: reghdfe wtp_diff false_pos false_neg ip_prot_diff ip_loss_diff, abs(subject_id) vce(cluster subject_id treatn)
    estadd scalar F_value = e(F)
	test false_pos = false_neg
	estadd scalar test_eq=r(p)
	testparm *
...
eststo: reghdfe wtp_diff i.risk_pref##c.false_pos i.risk_pref##c.false_neg ip_prot_diff ip_loss_diff, abs(subject_id) vce(cluster subject_id treatn)
    estadd scalar F_value = e(F)
	testparm *
    estadd scalar p_value=r(p)
	test false_pos = false_neg
...
eststo: reghdfe wtp_diff i.risk_pref##i.inac_bel2##c.false_pos i.risk_pref##i.inac_bel2##c.false_neg ip_prot_diff ip_loss_diff , abs(subject_id) vce(cluster subject_id treatn)
    estadd scalar F_value = e(F)
		testparm *
    estadd scalar p_value=r(p)
	test false_pos = false_neg
...
eststo: reghdfe wtp_diff i.risk_pref##i.inac_bel2##c.false_pos i.risk_pref##i.inac_bel2##c.false_neg  ip_prot_diff ip_loss_diff if !phigh, abs(subject_id plevel) vce(cluster subject_id treatn)
    estadd scalar F_value = e(F)
		testparm *
    estadd scalar p_value=r(p)
		test false_pos = false_neg
...
eststo: reghdfe wtp_diff false_pos false_neg consistent_wtp#c.false_pos consistent_wtp#c.false_neg, abs(subject_id) vce(cluster subject_id treatn)
    estadd scalar F_value = e(F)
	testparm *
    estadd scalar p_value=r(p)
	test false_pos = false_neg
...
eststo: reghdfe wtp_diff false_pos false_neg consistent_wtp#c.false_pos consistent_wtp#c.false_neg if !phigh, abs(subject_id plevel) vce(cluster subject_id treatn)
    estadd scalar F_value = e(F)
		testparm *
    estadd scalar p_value=r(p)
		test false_pos = false_neg
...
#delimit ;
	esttab * using "./Tables/wtpdiff_ols_consistency.tex", replace label 
		...
		label("\midrule $ R^2$" "Obs"   
		"[1em] Consistent Subjects: \\ \hspace{0.5em} False Positive" "\hspace{1em}  se" "\hspace{1em}  $ p$-value"  "[0.5em] \hspace{0.5em} False Negative" "\hspace{1em}  se" "\hspace{1em}  $ p$-value" ) 
		fmt(3 0 3 3 3 3 3 3) layout(@ @ @ (@) [@] @ (@) [@]))
		substitute(_ \_ ) style(tex);
#delimit cr
...
**Baseline WTP difference (full printed table): risk-aversion and belief accuracy
set more off
eststo clear
...
#delimit ;
	esttab * using "./Tables/wtpdiff_ols_full.tex", replace label 
		order(false_pos false_neg 2.risk_pref#c.false_pos 2.risk_pref#c.false_neg 1.risk_pref#c.false_pos 1.risk_pref#c.false_neg)
		cells(b(fmt(3)) se(par star fmt(3))) fragment booktabs 
		starlevels(* 0.1 ** 0.05 *** 0.01) varwidth(15) mlabels(, none) collabels(, none) noobs nonum noline noomit
		substitute(_ \_ ) style(tex);
#delimit cr
...
*Estimating the model with Tobit to account for censoring
set more off
eststo clear

eststo: tobit wtp false_pos false_neg, ll(0) ul(5) vce(cluster subject_id)
	test false_pos = false_neg
	estadd scalar test_eq=r(p)
	testparm *
    estadd scalar p_value=r(p)
	
eststo: tobit wtp i.risk_pref##c.false_pos i.risk_pref##c.false_neg i.subject_id, ll(0) ul(5) vce(cluster subject_id)
	test false_pos = false_neg
		estadd scalar test_eq=r(p)
	lincom false_pos+1.risk_pref#c.false_pos+1
		estadd scalar b_RL_fp_X=r(estimate)
		estadd scalar se_RL_fp_X=r(se)
		estadd scalar p_RL_fp_X=2*ttail(r(df),abs(r(estimate)/r(se)))
	lincom false_neg+1.risk_pref#c.false_neg+1
		estadd scalar b_RL_fn_X=r(estimate)
		estadd scalar se_RL_fn_X=r(se)
		estadd scalar p_RL_fn_X=2*ttail(r(df),abs(r(estimate)/r(se)))
	lincom false_pos+2.risk_pref#c.false_pos+1
		estadd scalar b_RA_fp_X=r(estimate)
		estadd scalar se_RA_fp_X=r(se)
		estadd scalar p_RA_fp_X=2*ttail(r(df),abs(r(estimate)/r(se)))
	lincom false_neg+2.risk_pref#c.false_neg+1
		estadd scalar b_RA_fn_X=r(estimate)
		estadd scalar se_RA_fn_X=r(se)
		estadd scalar p_RA_fn_X=2*ttail(r(df),abs(r(estimate)/r(se)))


eststo: tobit wtp i.risk_pref##i.inac_bel2##c.false_pos i.risk_pref##i.inac_bel2##c.false_neg i.subject_id , ll(0) ul(5) vce(cluster subject_id)
	test false_pos = false_neg
		estadd scalar test_eq=r(p)
	lincom false_pos+1.risk_pref#c.false_pos+1
		estadd scalar b_RL_fp_X=r(estimate)
...
eststo: tobit wtp i.risk_pref##i.inac_bel2##c.false_pos i.risk_pref##i.inac_bel2##c.false_neg i.subject_id if !phigh, ll(0) ul(5) vce(cluster subject_id)
		test false_pos = false_neg
		estadd scalar test_eq=r(p)
	lincom false_pos+1.risk_pref#c.false_pos+1
		estadd scalar b_RL_fp_X=r(estimate)
...
#delimit ;
	esttab * using "./Tables/wtp_tobit.tex", replace label 
		...
		cells(b(fmt(3)) se(par star fmt(3))) fragment booktabs 
		starlevels(* 0.1 ** 0.05 *** 0.01) varwidth(15) mlabels(, none) collabels(, none) nomtitle noobs nonum noline noomit
		stat(test_eq N b_RA_fp_X se_RA_fp_X p_RA_fp_X b_RA_fn_X se_RA_fn_X p_RA_fn_X b_RL_fp_X se_RL_fp_X p_RL_fp_X b_RL_fn_X se_RL_fn_X p_RL_fn_X ,  
		label("Prob(FP=FN)" "Obs"  "[1em] Risk-Averse Subjects: \\ \hspace{0.5em} False Positive" "\hspace{1em}  se" "\hspace{1em} $ p$-value" "[0.5em] \hspace{0.5em} False Negative" "\hspace{1em}  se" "\hspace{1em}  $ p$-value" 
		"\\[1em] Risk-Loving Subjects: \\ \hspace{0.5em} False Positive" "\hspace{1em}  se" "\hspace{1em}  $ p$-value"  "\\[0.5em] \hspace{0.5em} False Negative" "\hspace{1em}  se" "\hspace{1em}  $ p$-value" ) 
...
**Study IP bias effects using ip_val variable (projected value of losses

set more off
eststo clear

eststo: tobit wtp ip_val false_pos false_neg, ll(0) ul(5) vce(cluster subject_id)
	test false_pos = false_neg
	estadd scalar test_eq=r(p)
	testparm *
    estadd scalar p_value=r(p)
	
eststo: tobit wtp ip_val i.risk_pref##c.false_pos i.risk_pref##c.false_neg i.subject_id, ll(0) ul(5) vce(cluster subject_id)
	test false_pos = false_neg
		estadd scalar test_eq=r(p)
	lincom false_pos+1.risk_pref#c.false_pos+1
		estadd scalar b_RL_fp_X=r(estimate)
...
eststo: tobit wtp ip_val i.risk_pref##i.inac_bel2##c.false_pos i.risk_pref##i.inac_bel2##c.false_neg i.subject_id , ll(0) ul(5) vce(cluster subject_id)
	test false_pos = false_neg
		estadd scalar test_eq=r(p)
	lincom false_pos+1.risk_pref#c.false_pos+1
		estadd scalar b_RL_fp_X=r(estimate)
...
eststo: tobit wtp ip_val i.risk_pref##i.inac_bel2##c.false_pos i.risk_pref##i.inac_bel2##c.false_neg i.subject_id if !phigh, ll(0) ul(5) vce(cluster subject_id)
		test false_pos = false_neg
		estadd scalar test_eq=r(p)
	lincom false_pos+1.risk_pref#c.false_pos+1
		estadd scalar b_RL_fp_X=r(estimate)
...
#delimit ;
	esttab * using "./Tables/wtp_tobit_valdiff.tex", replace label 
		drop(?.risk_pref 0.inac_bel2 0.risk_pref#* ?.inac_bel2* ?.risk_pref#*inac* *3.risk_pref*)
		indicate(*.subject_id)
		order(ip_val false_pos false_neg 2.risk_pref#c.false_pos 2.risk_pref#c.false_neg 1.risk_pref#c.false_pos 1.risk_pref#c.false_neg)
		cells(b(fmt(3)) se(par star fmt(3))) fragment booktabs 
		starlevels(* 0.1 ** 0.05 *** 0.01) varwidth(15) mlabels(, none) collabels(, none) nomtitle noobs nonum noline noomit
		stat(test_eq N b_RA_fp_X se_RA_fp_X p_RA_fp_X b_RA_fn_X se_RA_fn_X p_RA_fn_X b_RL_fp_X se_RL_fp_X p_RL_fp_X b_RL_fn_X se_RL_fn_X p_RL_fn_X ,  
		label("Prob(FP=FN)" "Obs"  "[1em] Risk-Averse Subjects: \\ \hspace{0.5em} False Positive" "\hspace{1em}  se" "\hspace{1em} $ p$-value" "[0.5em] \hspace{0.5em} False Negative" "\hspace{1em}  se" "\hspace{1em}  $ p$-value" 
...
replace correctcrt=. if wave==1
gen crt_errors=7-correctcrt
sum crt_errors
gen good_crt=crt_errors<=3&!missing(crt_errors)
replace good_crt=. if wave==1
tab good_crt

gen high_gpa=gpa>3.5
replace high_gpa=. if missing(gpa)
tab high_gpa


label var high_gpa "GPA$>$3.5"
label var good_crt "$<$4 CRT errors"



eststo clear
eststo: ivreg2 wtp_diff i.high_gpa##c.false_pos i.high_gpa##c.false_neg high_gpa, cluster(subject_id treatn)
eststo: ivreg2 wtp_diff i.high_gpa##i.plevel i.high_gpa##c.false_pos i.high_gpa##c.false_neg high_gpa, cluster(subject_id treatn)
eststo: ivreg2 wtp_diff i.good_crt##c.false_pos i.good_crt##c.false_neg crt_errors, cluster(subject_id treatn)
eststo: ivreg2 wtp_diff i.good_crt##i.plevel i.good_crt##c.false_pos i.good_crt##c.false_neg crt_errors, cluster(subject_id treatn)
eststo: ivreg2 wtp_diff i.stat_educ##c.false_pos i.stat_educ##c.false_neg, cluster(subject_id treatn)
eststo: ivreg2 wtp_diff i.stat_educ##i.plevel i.stat_educ##c.false_pos i.stat_educ##c.false_neg, cluster(subject_id treatn)
esttab using "./Tables/table_wtpdiff_crt.tex", b(%9.3g) se(%9.1f) ar2(%9.2f) label indicate(Prior dummies=*.plevel) mtitles("" "" "" "" "" "" "" "" "") star("*" 0.10 "**" 0.05 "***" 0.01) nobaselevels compress nogaps replace



eststo clear
eststo: tobit wtp i.plevel sex stat_educ ncorrect,  ll(0) ul(5) vce(cluster subject_id)
eststo: tobit wtp i.plevel sex stat_educ ncorrect correctcrt gpa,  ll(0) ul(5) vce(cluster subject_id)
eststo: tobit wtp i.plevel false_pos false_neg i.sex#c.false_pos i.sex#c.false_neg sex stat_educ ncorrect,  ll(0) ul(5) vce(cluster subject_id)
eststo: tobit wtp i.plevel false_pos false_neg i.goodquiz#c.false_pos i.goodquiz#c.false_neg sex stat_educ ncorrect,  ll(0) ul(5) vce(cluster subject_id)
eststo: tobit wtp i.plevel false_pos false_neg i.stat_educ#c.false_pos i.stat_educ#c.false_neg sex stat_educ ncorrect, ll(0) ul(5)  vce(cluster subject_id)
eststo: tobit wtp i.plevel false_pos false_neg i.high_gpa#c.false_pos i.high_gpa#c.false_neg sex stat_educ correctcrt gpa ncorrect, ll(0) ul(5)  vce(cluster subject_id)
eststo: tobit wtp i.plevel false_pos false_neg i.good_crt#c.false_pos i.good_crt#c.false_neg sex stat_educ correctcrt gpa ncorrect, ll(0) ul(5) vce(cluster subject_id)
esttab using "./Tables/table_tobit_demo.tex", b(%9.3g) se(%9.1f) ar2(%9.2f) label indicate(Prior dummies=*.plevel) mtitles("" "" "" "" "" "" "") star("*" 0.10 "**" 0.05 "***" 0.01) nobaselevels compress nogaps replace
esttab using "./Tables/table_tobit_demo_pres.tex", cells("b(fmt(%9.3g) star)") ar2(%9.2f) label indicate(Prior dummies=*.plevel) mtitles("" "" "" "" "" "" "") star("*" 0.10 "**" 0.05 "***" 0.01) nobaselevels compress nogaps replace
...
*Analyzing the effects of errors for the round on wtp
*eststo: reghdfe wtp_diff false_pos false_neg, abs(subject_id) vce(cluster subject_id treatn)

label var absbel_err "Subject-round-specific belief error"
eststo clear
eststo: reghdfe wtp_diff absbel_err i.plevel, abs(subject_id) vce(cluster subject_id treatn)
eststo: reghdfe wtp_diff absbel_err false_pos false_neg i.plevel, abs(subject_id) vce(cluster subject_id treatn)
...
*eststo clear
*eststo: reghdfe wtp_diff false_pos false_neg i.plevel if inconsistent_wtp==0, abs(subject_id) vce(cluster subject_id treatn)
*eststo: reghdfe wtp_diff false_pos false_neg if plevel==100&nchanges>0, abs(subject_id) vce(cluster subject_id treatn)
*eststo: reghdfe wtp_diff false_pos false_neg if plevel==200&nchanges>0, abs(subject_id) vce(cluster subject_id treatn)
*eststo: reghdfe wtp_diff false_pos false_neg if plevel==300&nchanges>0, abs(subject_id) vce(cluster subject_id treatn)
*eststo: reghdfe wtp_diff phintBW phintWB if plevel==500&nchanges>0, abs(subject_id) vce(cluster subject_id treatn)
...
eststo clear
eststo: ivreg2 wtp_diff false_pos false_neg i. sex i.sex#c.false_pos i.sex#c.false_neg, cluster(subject_id treatn)
eststo: ivreg2 wtp_diff i.sex##i.plevel i.sex##c.false_pos i.sex##c.false_neg, cluster(subject_id treatn)
eststo: ivreg2 wtp_diff i.stat_educ##c.false_pos i.stat_educ##c.false_neg, cluster(subject_id treatn)
eststo: ivreg2 wtp_diff i.stat_educ##i.plevel i.stat_educ##c.false_pos i.stat_educ##c.false_neg, cluster(subject_id treatn)
...
bysort treatn: gen n_treat = _N
gen w_treat = 1/n_treat
gen wtp_diffw = w_treat*wtp_diff
...
forvalues i=0/1{
  forvalues j=0/1{
    ...
    * denominator: total weight in cell
    sum w_treat if fp_env==`i' & fn_env==`j', meanonly
    local totweights = r(sum)

    * equal-treatment mean of wtp_diff
    sum wtp_diffw if fp_env==`i' & fn_env==`j', meanonly
    local wtp_diff = r(sum)/`totweights'

    * clustered test of E[wtp_diff]=0 (one-sample)
    gen d0 = wtp_diff
...
forvalues k=0/1{
  forvalues i=0/1{
    forvalues j=0/1{
      ...
      * denominator: total weight in cell
      sum w_treat if highprob==`k' & fp_env==`i' & fn_env==`j', meanonly
      local totweights = r(sum)

      * equal-treatment mean of wtp_diff
      sum wtp_diffw if highprob==`k' & fp_env==`i' & fn_env==`j', meanonly
      local wtp_diff = r(sum)/`totweights'

      * clustered test of E[wtp_diff]=0 (one-sample)
      gen d0 = wtp_diff
      reg d0 [pw=w_treat] if highprob==`k' & fp_env==`i' & fn_env==`j', ///
          vce(cluster treatn)
      test _cons = 0
      local ptest = r(p)
      drop d0
...
eststo clear
eststo: reghdfe wtp_diff phintBW phintWB if nchanges>0, abs(subject_id) vce(cluster subject_id treatn)
eststo: reghdfe wtp_diff phintBW phintWB if plevel==100&nchanges>0, abs(subject_id) vce(cluster subject_id treatn)
eststo: reghdfe wtp_diff phintBW phintWB if plevel==200&nchanges>0, abs(subject_id) vce(cluster subject_id treatn)
eststo: reghdfe wtp_diff phintBW phintWB if plevel==300&nchanges>0, abs(subject_id) vce(cluster subject_id treatn)
eststo: reghdfe wtp_diff phintBW phintWB if plevel==500&nchanges>0, abs(subject_id) vce(cluster subject_id treatn)
esttab using "./Tables/table_wtpdiff_06s.tex", b(%9.3g) se(%9.1f) ar2(%9.2f) label title(WTP - Value of Information, by prior) mtitles("All" "0.1" "0.2" "0.3" "0.5")  addnotes("Only subjects who change their decisions across priors") star("*" 0.10 "**" 0.05 "***" 0.01) nobaselevels compress nogaps replace
...
*Testing if changing choices when changing priors:
sort subject_id round
cap drop noprior_response
cap drop anchored

gen noprior_response=wtp==wtp[_n-1]&round==4
tab noprior_response

bys wave: tab noprior_response

bys subject_id: egen anchored=sum(noprior_response)
tab anchored




*comparing symmetric treatments (differing only by relabeling FP to FN)
use "./Temp/wtp_discrepancy0.dta", replace
*first, creating a frame of treatments to match
keep plevel phintWB phintBW treatn wtp round subject_id value
rename treatn treatn_match
rename wtp wtp2
rename value value2
rename round round2

gen phintWB2=phintWB
gen phintBW2=phintBW
gen phintWBr=round(100*phintBW2)
gen phintBWr=round(100*phintWB2)

drop if (phintWB==0) & (phintBW==0)
drop phintWB phintBW

drop if phintWBr==phintBWr

save "./Temp/mirror_treatments.dta", replace



use "./Temp/wtp_discrepancy0.dta", replace
gen phintWBr=round(100*phintWB)
gen phintBWr=round(100*phintBW)
merge 1:1 phintWBr phintBWr plevel subject_id using "./Temp/mirror_treatments.dta"

keep if _merge==3

gen long pair_id = cond(treatn < treatn_match, treatn, treatn_match)
tab pair_id


drop if treatn == treatn_match 

keep subject_id round value value2 wtp wtp2 phintWBr phintBWr phintWB phintBW phintWB2 phintBW2 p plevel treatn pair_id
order subject_id round value value2 wtp wtp2 phintWBr phintBWr phintWB phintBW phintWB2 phintBW2 p plevel treatn pair_id


*finding only the differences with positive WTP:
gen value_min = min(value, value2)
gen extra_pair=value>value_min
...
tab phintWB phintBW
drop if value>value_min
tab phintWB phintBW

tab pair_id

*dropping mirror treatments if they have the same wtp but different inform structures
*gen phintWBmin = min(phintWBr, phintBWr)
*drop if phintWBr>phintWBmin

gen delta=wtp2-wtp
sum delta


ivreg2 delta i.treatn, nocons cluster(subject_id)
testparm i.treatn
