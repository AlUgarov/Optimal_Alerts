...
local p=r(p)
estadd scalar p = `p'
esttab using "./Tables/wtp_het_test.tex", b(%9.3g) se(%9.3f) ar2(%9.2f) label drop(_cons fp_cost fn_cost *.plevel) stats(p adj_r N, labels("P(beta_X=0)" "Adjusted \(R^{2}\)" "Observations")) title(WTP for Information (OLS): analyze FP/FN sensitivity by prior) mtitles("" "" "" "FE" "FE" "FE") nobaselevels nostar compress nogaps replace
esttab using "./Tables/wtp_het_test_pres.tex", b(%9.3g) se(%9.3f) ar2(%9.2f) label drop(_cons fp_cost fn_cost *.plevel) stats(p adj_r N, labels("P(beta_X=0)" "Adjusted \(R^{2}\)" "Observations")) addnotes("Controlling for FP/FN rates, prior, constant omitted") mtitles("" "" "" "FE" "FE" "FE") nobaselevels nostar compress nogaps replace
...
estadd scalar adj_r= e(r2_a)
estadd scalar p = r(p)
esttab using "./Tables/wtp_fpvsfn_test.tex", b(%9.3g) se(%9.3f) ar2(%9.2f) label drop(_cons *.plevel) stats(p adj_r N, labels("P(FP rate=FN rate)" "Adjusted \(R^{2}\)" "Observations")) title(WTP for Information (OLS): comparing sensitivity to FP/FN rates) mtitles("" "" "FE" "FE") nobaselevels nostar compress nogaps replace
esttab using "./Tables/wtp_fpvsfn_test_pres.tex", b(%9.3g) se(%9.3f) ar2(%9.2f) label drop(_cons *.plevel) stats(p adj_r N, labels("P(FP rate=FN rate)" "Adjusted \(R^{2}\)" "Observations")) mtitles("" "" "FE" "FE") nobaselevels compress nogaps replace
...
#delimit ;
	esttab * using "./Tables/wtpdiff_ols.tex", replace label 
		drop(?.risk_pref 0.inac_bel2 0.risk_pref#* ?.inac_bel2* ?.risk_pref#*inac* *3.risk_pref*) nostar
		...
		cells(b(fmt(3)) se(par fmt(3))) fragment booktabs 
		nostar varwidth(15) mlabels(, none) collabels(, none) nomtitle noobs nonum noline noomit addnotes("Errors are clustered by subject") 
...
#delimit ;
	esttab * using "./Tables/wtpdiff_ols_full.tex", replace label 
		...
		nostar varwidth(15) mlabels(, none) collabels(, none) noobs nonum noline noomit
		substitute(_ \_ ) style(tex);
#delimit cr
...
listtex using "./Tables/bigpicture_wtp_det.tex", type rstyle(tabular) head("\begin{table}[H]\centering \caption{Average WTP discrepancy (WTP-Value) by Signal Type} \begin{tabular}{ccccc} \hline \hline" `"\textbf{Priors}&\textbf{Honest}&\textbf{FN only}& \textbf{FP only} & \textbf{FP and FN}\\ \hline"') foot("\hline \\ \end{tabular} \end{table}") replace
...
esttab using "./Tables/table_wtpdiff_06s.tex", b(%9.3g) se(%9.3f) ar2(%9.2f) label title(WTP - Value of Information, by prior) mtitles("All" "0.1" "0.2" "0.3" "0.5")  addnotes("Only subjects who change their decisions across priors") nobaselevels compress nogaps replace
...
esttab using "./Tables/table_wtpdiff_order.tex", b(%9.3g) se(%9.3f) ar2(%9.2f) label title(WTP - Value of Information, by prior with order effects) mtitles( "p=0.1,0.2" "p=0.3,0.5" "p=0.1,0.2" "" "" "") star("*" 0.10 "**" 0.05 "***" 0.01) nobaselevels compress nogaps replace
*/
...
esttab using "./Tables/table_wtpdiff_06.tex", b(%9.3g) se(%9.3f) ar2(%9.2f) label title(WTP - Value of Information, by prior) mtitles("All" "0.1" "0.2" "0.3" "0.5") star("*" 0.10 "**" 0.05 "***" 0.01) nobaselevels compress nogaps replace
*/
