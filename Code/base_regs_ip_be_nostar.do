...
estadd scalar p = r(p)
estadd scalar r2a = e(a_r)
esttab using "./Tables/table_ip0_lin.tex", b(%9.3g) se(%9.3f) ar2(%9.2f) label stats(p N r2a, labels("P(FP rate $\neq$ FN rate)" "Observations" "Adjusted \(R^{2}\)")) addnotes("Errors are clustered by subject.") indicate(Subject FE = *.subject_id) mtitles("All" "S=White" "S=Black" "All" "S=White" "W=Black") title(Informed protection response: linear probability regression) nostar nobaselevels compress nogaps replace
...
esttab m1 m2 m3 m4 using "./Tables/table_ip_flex.tex", b(%9.3f) se(%9.3f) label addnotes("With flexible controls of posterior probability and beliefs" ///
  "Subject FE, errors are clustered by subject, average marginal treatment effects") stats(N r2p llike, labels("N" "Pseudo R-squared" "Log-likelihood") fmt(0 3 3)) mtitles("Posterior only" "Posterior only" "Both" "Both") title(Informed Protection Response: flexible control for posteriors and beliefs) nostar nobaselevels compress nogaps replace
...
esttab using "./Tables/table_ip5_semi.tex", b(%9.3g) se(%9.3f) ar2(%9.2f) label mtitles("" "" "" "") title(Informed protection response: semiparametric control for posteriors) nostar nobaselevels compress nogaps replace
...
esttab using "./Tables/table_be_err.tex", b(%9.3f) se(%9.3f) ar2(%9.3f) label addnotes(Dep. variable: reported belief - posterior probability) title(Belief Elicitation: When Mistakes Happen) mtitles("All" "S=White" "S=Black") nostar nobaselevels compress nogaps replace
...
esttab using "./Tables/table_be_err1.tex", b(%9.3g) se(%9.3f) ar2(%9.2f) label addnotes(Dep. variable: reported belief - posterior probability) title(Belief Elicitation: When Mistakes Happen) mtitles("All" "S=White" "S=Black") nostar indicate(Subject FE = *.subject_id) nobaselevels compress nogaps replace
