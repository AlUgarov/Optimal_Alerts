...
eststo: logit ip_ phintBW phintWB blackhint post_prob, vce(cluster subject_id)
eststo m3: margins, dydx(phintBW phintWB blackhint post_prob) post
eststo: logit ip_ phintBW phintWB fef i.subject_id, vce(cluster subject_id)
eststo m4: margins, dydx(phintBW phintWB) post
eststo: logit ip_ phintBW phintWB blackhint fef i.subject_id, vce(cluster subject_id)
eststo m5: margins, dydx(phintBW phintWB blackhint) post
eststo: logit ip_ phintBW phintWB blackhint post_prob fef i.subject_id, vce(cluster subject_id)
...
eststo: reg be_ post_prob blackhint false_prob if class==1, vce(cluster subject_id)
eststo: reg be_ post_prob blackhint false_prob if class==2, vce(cluster subject_id)
...
reg bel_err i.class_alt##c.phintWB i.class_alt##c.phintBW i.subject_id if blackhint==0, vce(cluster subject_id)
...
esttab using "./Tables/wtp_het_risk.tex", b(%9.3g) se(%9.1f) ar2(%9.2f) label indicate("Risk pref dummies = *.risk_pref") title(WTP minus Value of Information, risk aversion and sensitivity to FP and FN costs) mtitles("" "" "" "FE" "FE" "FE") star("*" 0.10 "**" 0.05 "***" 0.01) nobaselevels compress nogaps replace
esttab using "./Tables/wtp_het_risk_pres.tex", b(%9.3g) ar2(%9.2f) not label drop(_cons) indicate("Risk pref dummies = *.risk_pref") mtitles("" "" "FE" "FE") star("*" 0.10 "**" 0.05 "***" 0.01) nobaselevels compress nogaps replace





*drop p_wei false_pos_wei false_neg_wei cost_bp_wei freqBW_wei freqWB_wei false_pos_wei1 false_neg_wei1
**Generating weighted probability:
local alpha=0.1
*gen p_wei=exp(-(-ln(p))^`alpha')
*gen p_wei=0.3 if seq<4
*replace p_wei=0.2 if seq>3.01
gen p_wei=.
replace p_wei=0.2 if plevel==100&seq<4
replace p_wei=0.5 if plevel==300&seq<4
replace p_wei=0.1 if plevel==200&seq>3.01
replace p_wei=0.3 if plevel==500&seq>3.01
tab p p_wei

*Should I weight the whole probability/frequency?
gen freqBW_wei=exp(-(-ln(freqBW))^`alpha')
gen freqWB_wei=exp(-(-ln(freqWB))^`alpha')

gen false_pos_wei1=freqBW_wei*protectioncost //expected protection costs for false positives
gen false_neg_wei1=freqWB_wei*loss //expected false positive costs


gen false_pos_wei=(1-p_wei)*phintBW*protectioncost //expected protection costs for false positives
*gen false_pos_wei=(rev_p_wei*phintBW)*protectioncost //expected protection costs for false positives
gen false_neg_wei=p_wei*phintWB*loss //expected false positive costs
gen cost_bp_wei=min(p_wei*loss, protectioncost) //

**How to test if people indeed react to weighted probabiities?
reg wtp false_pos_wei false_neg_wei cost_bp
*reg wtp false_pos_wei1 false_neg_wei1 cost_bp
reg wtp false_pos false_neg cost_bp
...
gen false_prob=phintWB+phintBW
reg wtp i.plevel false_prob, vce(cluster subject_id)
reg wtp i.plevel phintWB phintBW, vce(cluster subject_id)
reg wtp i.plevel##c.phintWB i.plevel##c.phintBW, vce(cluster subject_id)



eststo: reg wtp i.highprob##c.false_neg_wei i.highprob##c.false_pos_wei, vce(cluster subject_id)
eststo: reg wtp i.highprob##c.false_neg i.highprob##c.false_pos, abs(subject_id) vce(cluster subject_id)
eststo: reg wtp i.highprob##c.false_neg_wei i.highprob##c.false_pos_wei, abs(subject_id) vce(cluster subject_id)
esttab using "./Tables/table_wtp_pweihgt.tex", b(%9.3g) se(%9.1f) ar2(%9.2f) label title(WTP for Information (OLS) prior heterogeneity with probability weighting) mtitles("" "" "" "" "") star("*" 0.10 "**" 0.05 "***" 0.01) nobaselevels compress nogaps replace




reg wtp  i.plevel i.phintBWcat i.phintWBcat
drop false_cost phintWBcat phintBWcat fn_cost fp_cost
*Sensitivity to priors vs sensitivity to FP/FN rates




*How do I build an estimation here? Weighted probabilities have more explanatory power and reduce the heterogeneity
*Use just the interaction for higher priors? Show that this interaction is insignificant when using the weighted probabilities

eststo clear
eststo: reg wtp i.highprob##c.false_neg i.highprob##c.false_pos, vce(cluster subject_id)
eststo: reg wtp i.highprob##c.false_neg_wei i.highprob##c.false_pos_wei, vce(cluster subject_id)
eststo: reg wtp i.highprob##c.false_neg i.highprob##c.false_pos, abs(subject_id) vce(cluster subject_id)
eststo: reg wtp i.highprob##c.false_neg_wei i.highprob##c.false_pos_wei, abs(subject_id) vce(cluster subject_id)
esttab using "./Tables/table_wtp_pweihgt.tex", b(%9.3g) se(%9.1f) ar2(%9.2f) label title(WTP for Information (OLS) prior heterogeneity with probability weighting) mtitles("" "" "" "" "") star("*" 0.10 "**" 0.05 "***" 0.01) nobaselevels compress nogaps replace
...
eststo clear
eststo: reg wtp_diff i.strategy_short  phintBW phintWB
eststo: reg wtp_diff i.strategy_short##c.phintBW i.strategy_short##c.phintWB
...
**Baseline WTP differences:
reg wtp_diff false_tot, vce(cluster subject_id)
...
esttab using "./Tables/table_wtpdiff_04tob_pres.tex", b(%9.3g) se(%9.1f) ar2(%9.2f) drop(_cons) eqlabels(none) label mtitles("0.1" "0.2" "0.3" "0.5") addnotes("Tobit regression of WTP, constant omitted") star("*" 0.10 "**" 0.05 "***" 0.01) nobaselevels compress nogaps replace
