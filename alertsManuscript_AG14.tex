...
\maketitle
\onehalfspacing
\begin{abstract}{%Designing multiple kinds of signals involves trade-offs between false-positive and false-negative costs. 
...
To understand preferences over these trade-offs, we study the demand for information in the framework with a potential protection action that stylizes the aforementioned scenarios. The subject first receives a signal about the probability of an adverse event, then decides whether or not to protect. We find that willingness-to-pay (WTP) has only weak correlation with the value of information. There are three main findings. First, subjects on average underreact to the quality of the signal, resulting in overpaying for low-quality signals and underpaying for high-quality signals. Second, subjects tend to overreact to false-negative rates when the prior probability is low and overreact to false-positive positive rates when priors are high. Finally, we show that this pattern is most consistent with a failure to estimate the effect of the frequencies of false-positive and false-negative outcomes on the potential costs of using the signal. 
...
{\color{red}
\begin{itemize}
	\item We need to explicitly state how we contribute to the each of the literature we claimed to contribute to: what's the novel aspect, method or results?
	\item The paragraph that starts with ``Our work contributes to ... of diagnostic tests or alarms'' is long, but I feel it could be more concise. We should procure findings of the studies that specifically linked to our results, but are somewhat inadequate --- and how our study fills in this gap.
	\item Similarly, the one previous to that para can also be made more concise and if possible, add how our results contribute to the literature.
\end{itemize}
}
...
%
%\aust{Our results provide some theoretical guidelines on the design of future alerts systems. Our paper indicates that users are not sensitive enough to the accuracy of alert signals, sometimes even paying for tests having no effect on their decisions. This is consistent with many real-world systems having too high false-positive rates which can result in alarm fatigue. As this bias is most consistent with failure of probabilistic reasoning, designers of alerts systems should give more attention to how the information on their accuracy is presented to their main users.}
