...
We use the BP, IP, and BE tasks to construct measures of the different components of WTP. Using BP, we construct a measure subjects' responses to the prior and their risk preference. We use the IP task to examine how signals affect protection decisions. Finally, we use the BE task to construct a measure of their Bayesian updating ability given signal quality. Table~\ref{tab:treatments} shows the values of the different priors in our treatments, as well as the gremlin groupings (along with the associated false positive and false positive rates) that we used for the different tasks.
...
\subsection{Willingness to Pay for Signals} 
...
\noindent\agt{XXX Is a false positive event = having a black gremlin in the group? Should we write false positive rate or is there a distinction you want to highlight? XXX}
