...
Subjects receive more information in the IP task than in BP task, , though using that information requires that subjects engage in Bayesian updating.  Figure~\ref{fig:ProtResponse} shows that, consistent with their behavior in the BP task, the share of subjects protecting in the IP task is increasing in the posterior probabilities. Behavior is noisier, likely due to the fact that subjects have to calculate the posterior.\pmt{I'm not sure what we mean by ``noisier" here.  Is it more overall switching?  Within individual inconsistencies?  Greater discrepancies between posteriors at which they switch?}  At the individual level, we also observe a significant correlation between protection responses made by subjects in BP and IP tasks. The number of protection choices made in BP positively correlates with protection choice in the TP task.\pmt{Is this sentence saying the same thing as the next one?  If not, then I don't understand what we are measuring here.  In either case, we need to put a number to it (correlation and the p-value).  I presume we have more inconsistent decision makers here?  It's worth putting a number to that, too, as we did for BP.  Are inconsistent BP guys more likely to be inconsistent here? }

Subjects' responses in the first two tasks suggest that conditional on the true probabilities, subjects protected more in the IP task compared to the BP task at low probabilities, \aut{even though the difference is not statistically significant}\pmt{What is the test? Proportion of protection decisions for probabilities less than X or something?}. These choices suggest that subjects’ updated beliefs overshot the true posterior at low probabilities.  It is difficult to make make general statements about this observation because the BP task did not cover the full range of probabilities observed in the IP task, but it may be consistent with ambiguity aversion in the IP task.\pmt{Do we want to bring up ambiguity aversion?  Do we discuss it again anywhere?  If not, it's probably better just to leave it out.}
...
Panel A of Figure~\ref{fig:BeliefUpdate} uses all elicted beliefs and suggests that, while errors occur, beliefs are still sensible. The distribution of updating errors is centered at 0, with \pmt{90\% of errors between XX and YY OR XX\% of errors between +/-0.1 (or something similar)}.  Overall, the correlation between elicited belief and the true posterior was 0.653.  Using all observations, however, has the potential to obscure an important distinction: some posteriors are uncertain, but some are certain.  In Panel B of Figure~\ref{fig:BeliefUpdate} includes only those beliefs elicited for an uncertain posterior, \pmt{which comprises xx\% of all elicted beliefs}. \pmt{We can see that the median error is XX, with with 90\% of errors between XX and YY}, suggesting that subjects tend to overestimate the likelihood of adverse events for uncertain posteriors, \pmt{which is consistent with what we see in Figure~\ref{fig:ProtResponse}}. The correlation between beliefs and posteriors in this subset of observations is only 0.571.  Panel C of Figure~\ref{fig:BeliefUpdate} plots the distribution of updating errors with certain posteriors, which includes: (i) treatments with all-honest gremlins; and (ii) treatments with obviously irrelevant dishonest gremlins (e.g., a group with honest and white-eyed gremlins with a hint that the ball is black — or vice versa). Reassuringly, \pmt{XX\%} of the errors are 0, but errors are non-zero for \pmt{XX\%} 20 percent of the time\pmt{Am I missing a distinction? In the figure it looks like the mass at 0 is maybe 70-75\%, suggesting that it's more than 20\%}. About half of these involve reporting a probability of between one and zero, with the other half reporting a probability of one when it should have been zero. For the latter case, only 51 percent of responses were perfectly correct\pmt{What does it mean that these are ``perfectly correct"}.\aut{How do we address the issue of knuckeheads here making errors with certain posteriors? Excluding them and re-calculating?}\pmt{It depends a bit.  Are some guys doing everything right here and others doing everything wrong?  Is there any relationship between errors here and inconsistencies in BP or a lower correlation between BP and IP tasks?} 
