...
\begin{frame}
\frametitle{Protection Summary detail}
\begin{itemize}
...
\footnotesize
\begin{table}[H]\centering
\caption{Average Protection by Signal Type (Split by Priors)}
\label{tab:nonparIP}
\adjustbox{max width=\textwidth}{
\begin{threeparttable}
...
\hline\hline
\multirow{4}{6ex}{\centering \textbf{Row}}
& \multirow{3}{10ex}{\centering \textbf{Priors}}
& \multicolumn{2}{c}{\centering \textbf{Signal Characteristics}}
& \multirow{3}{12ex}{\centering \textbf{Hint}}
...
& (1) & (2) & (3) & (4) & (5) & (6) & (7) & (8) \\
\hline
\input{Tables/bigpicture_IPd_AU.tex}
\\[-1em]
\hline\hline
\end{tabular}

\begin{tablenotes}[flushleft]
\item\footnotesize \textit{Notes: Column (8) reports the p-value for the test of equality between the theoretical prediction (Share Optimal) and the observed share of protection (Share Protect). Priors are grouped as Low (plevel 100/200) vs High (plevel 300/500).}
\end{tablenotes}
\end{threeparttable}
}
\end{table}
...
\footnotesize
\begin{table}[H]\centering
\caption{IP decisions (actual minus optimal risk-neutral)}
\label{tab:nonparIP}
\input{Tables/bigpicture_IPdiff_compact_AU.tex}
\begin{tablenotes}[flushleft]
\item\footnotesize \textit{Notes: Priors are grouped as Low (plevel 100/200) vs High (plevel 300/500).}
\end{tablenotes}
\end{table}
\end{frame}
...
\begin{frame}
\frametitle{Belief Elicitation detail}

\footnotesize
\begin{table}[H]\centering
\caption{Belief Elicitaion summary  (reported minus posterior)}
\label{tab:nonparIP}
\input{Tables/bigpicture_BEdiff_compact_AU.tex}
\begin{tablenotes}[flushleft]
\item\footnotesize \textit{Notes: Priors are grouped as Low (plevel 100/200) vs High (plevel 300/500).}
\end{tablenotes}
\end{table}
...
\begin{itemize}
\item We can never reject the hypothesis that empirical FP/FN sensitivities are the same, but in 1/2 cases cannot reject that they are equal to theoretical ones.
\end{itemize}
\begin{figure}[h]
...
\begin{frame}
\frametitle{Comments (Reviewer 1): Task cross-consistency}
\footnotesize
\parbox{\textwidth}{

\textit{1) "For instance, a BE-IP consistency check could examine whether, given a threshold
posterior of 0.25, a consistent decision maker who reports posterior $>$ 0.25 in the BE
task also chooses Protect” in the IP task. Across the six tasks generating 12 posteriors,
the authors could identify decision thresholds for different subjects and inconsistency
measures at the individual level.”}
}
\par
\vspace{2em}
\parbox{\textwidth}{
\textit{2) "Similarly, an IP-WTP consistency check could examine whether participants who
chose to protect or not protect regardless of possible signals assign zero WTP to
information structures, as predicted by the assumption that access to a signal can
increase expected utility if the signal affects her protection decisions.” These analyses
would likely reveal that high-consistency subjects show clearer false positive/false
negative sensitivity patterns, while low-consistency subjects drive the null aggregate
results, explaining why their current pooled analysis fails to detect the theoretically
predicted effects."
}
}
...
\begin{frame}
\frametitle{Protection-Beliefs consistency}

\begin{itemize}
\item Measure the consistency of beliefs and protection decisions using subject-level \href{https://en.wikipedia.org/wiki/Mann-Whitney_U_test}{Mann-Whitney U-statistic}:
\begin{itemize}
\item Isomorphic to N of cases in which a subjects protects for belief $\mu_1$ but doesn't protect for belief $\mu_2\geq \mu_1$
\item Should be 1 for any rational subject regardless of risk preferences
\end{itemize}
\item Median U is 0.925 (pretty good, 1-2 discrepancies), 35\% of subjects are fully consistent, but 25\% of subjects has U$<$0.71
\item Inconsistent subjects have significantly lower belief accuracy (+60\% mean absolute error, p$<$0.001), and quiz (-1 question).
\item Seems to indicate that they are not confident in their answers and either do not take their beliefs into account or recalculate them when making protection decisions
\end{itemize}
...
\begin{frame}
\frametitle{Protection-WTP consistency}

\begin{itemize}
\item In most cases, subjects both react to a signal and indicate positive WTP, or do neither
\item However, in about 1/3 of cases their actions are misaligned for the task:
\end{itemize}
%\adjustbox{max width=0.9\textwidth}{
%\input{Tables/protection_to_wtp.tex }
%}
\begin{tabular}{lcccc}
\toprule
...
\begin{frame}
\frametitle{Consistent subjects are more sensitive to FN/FN costs}

\begin{itemize}
\item Consistent subjects do not show significant biases in either FP or FN sensitivity both on average and for low/high priors!
\end{itemize}
\begin{table}[htbp!]
\centering
\adjustbox{max width = 0.7\textwidth}{
	\begin{threeparttable}
	%\caption{Deviations from Signal Value (WTP - Value) and Protection Consistency}
	\begin{tabular}{l*{4}{c}}
		\hline\hline
		&\multicolumn{2}{c}{\multirow{2}{*}{All}} & \multicolumn{2}{c}{Prior}\\ \cmidrule(lr){4-5}
		&&& $\{.1,.2\}$ & $\{.3,.5\}$\\ 
%\begin{tabular}{lcccc}
\input{Tables/wtpdiff_ols_consistency.tex}
\end{tabular}
\end{threeparttable}
}
\end{table}
...
\begin{frame}
\frametitle{Potential implications for framing our results}
\begin{itemize}
\item Highly consistent subjects are still vulnerable to the same bias: underreacting to FP for low priors (when FP are very likely) and underreacting to FN for high priors.
\item Biases likely have cognitive origins - paying for a signal and not using it is hard to explain just from preferences
\item Supported by the observed high correlations between protection-belief inconsistency, WTP-protection inconsistency, belief accuracy, and quiz results
\end{itemize}
\end{frame}
...
\begin{frame}
\frametitle{Comments: Does WTP bias follows from IP bias?}

\parbox{\textwidth} {Reviewer 2 also wants more cross-task analysis but with a deeper question:}
\par
\vspace{1em}
\footnotesize
\parbox{\textwidth}{\textit{1) "However, the current version of the paper does not explore much of the relationship
...
\begin{frame}
\frametitle{What the Expected Bias Entails?}

\begin{itemize}
\item We can imagine three ways to be aware of your biases:
\begin{enumerate}
\item  I know my bias direction and magnitude give a signal ("Cannot resist hiding for that  tornado siren test on Wednesday"). 
\item  I know that I do not account for certain information when making a decision (eg.: failing to distinguish FP and FN rates).
\item I know that I poorly understand particular signal structures, so expect to make more errors and extract less value from these signals
\end{enumerate}
\item Reviewer's point: Subjects do not differentiate FP and FN costs in WTP $\implies$ What if they are aware of that bias in the IP too and pay less for signal structures known to suffer from this bias?
\end{itemize}
\end{frame}
...
\begin{itemize}
\item Biases of the first type are internally inconsistent and a priori implausible:
\begin{itemize}
\item Why not adjust the protection decision if knowing the bias direction?
\item Also, if a person is aware of equal sensitivity to FP/FN in the IP task, they would still have non-equal sensitivty in WTP task as long as they differentiate two errors there
\end{itemize}
\item Both types 2 and 3 imply poor use of signals. Possible test: do they pay less for signals in which they have large BE errors?
\item We can also write a simple model for type 2 bias by rewriting the signal purchase problem in terms of variables they do account for: $\pi$, proportion of dishonest gremlins.
\begin{itemize}
...
\begin{itemize}
\item Belief error in round indeed strongly correlates with WTP (with subject and prior FE) if controlling for FP/FN costs$\implies$ Consistent with bias awareness of type 2 or 3 
\end{itemize}
\begin{table}[htbp]
\caption{WTP minus Value of Information: subject-round BE error}
...
\begin{frame}
\frametitle{Some thoughts on bias awareness}
\begin{itemize}
\item There is an indication that subjects might be aware that they do not use some signals well and pay less for those
\item Being aware of your biases cannot explain paying for unused signals which we observe, and which is more consistent with reasoning problems
\item Given that the average WTP roughly equals the risk-neutral value, either the bias awareness has to be small, or risk preferences increase baseline WTP
\end{itemize}
\end{frame}
